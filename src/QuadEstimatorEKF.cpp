...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  // From "Estimation for Quadrotors". (Eq. 52)
  float theta = pitch;
	float phi = roll;
	float psi = yaw;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  // From: Estimation for Quadrotors - Eq. 51
	gPrime(0, 3) = dt;
	gPrime(1, 4) = dt;
	gPrime(2, 5) = dt;
	...
	// From: Estimation for Quadrotors - Section 3
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  zFromX(0) = ekfState(6);
  float yaw_est_error = magYaw - ekfState(6);

  // Normalize Yaw to pi
  if (yaw_est_error > F_PI)
  {
	  zFromX(0) += 2.f*F_PI;
  }
  else if (yaw_est_error < -F_PI)
  {
	  zFromX(0) -= 2.f*F_PI;
  }
  // Measurement matrix derivative
  hPrime(0,6) = 1;
  /////////////////////////////// END STUDENT CODE ////////////////////////////
